...
{
    class M4APlayer : IPlayer
    {
        readonly M4AMixer mixer;
        readonly byte[] rom;
        readonly EndianBinaryReader reader;
        readonly TimeBarrier time;
        readonly Thread thread;
        int voiceTableOffset;
        Track[] tracks;
        ushort tempo;
        int tempoStack;
        ...
        {
            const int songTableOffset = 0x1E866BC; // TODO
            var entry = reader.ReadObject<SongEntry>(songTableOffset + (index * 8));
            var header = reader.ReadObject<SongHeader>(entry.HeaderOffset - GBAUtils.CartridgeOffset);
        ...
        void PlayNote(Track track, byte key, byte velocity, byte addedDuration)
        {
        ...
        void ExecuteNext(Track track, ref bool update)
        {
