...
public static class AlphaDreamSoundFontSaver_DLS
{
	// Since every key will use the same articulation data, just store one instance
	private static readonly Level2ArticulatorChunk _art2 = new()
	{
	...
	private static void AddInfo(AlphaDreamConfig config, DLS dls)
	{
		...
		info.Add(new InfoSubChunk("ISFT", ConfigUtils.PROGRAM_NAME));
	}

	private static Dictionary<int, (WaveSampleChunk, int)> AddSamples(AlphaDreamConfig config, DLS dls)
	{
		...
		for (int i = 0; i < config.SampleTableSize; i++)
		{
			...
			if (sh.DoesLoop == SampleHeader.LOOP_TRUE)
			{
	...
	private static void AddInstruments(AlphaDreamConfig config, DLS dls, Dictionary<int, (WaveSampleChunk, int)> sampleDict)
	{
		...
		for (int v = 0; v < 256; v++)
		{
			...
			ins.Add(new InstrumentHeaderChunk(new MIDILocale(0, (byte)(v / 128), false, (byte)(v % 128)))
			{
